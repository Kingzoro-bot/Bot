...
var videotime = 2000;
const { cmd } = require("../lib/plugins");// To manage file paths
smd({
  'pattern': "play",
  'alias': ["music"],
  ...
  try {
    ...
    const _0x23d6e1 = "https://giftedapis.us.kg/api/download/ytmp3v2?url=" + _0x4f86cb.url + "&apikey=gifteddevskk";
    let _0x4acf6c = 3;
    while (_0x4acf6c > 0) {
      try {
...
smd({
  ...
  try {
    ...
    const _0x2e06bc = "https://giftedapis.us.kg/api/download/ytmp3v2?url=" + _0x2f47ed + "&apikey=gifteddevskk";
    let _0x5f0163 = 3;
    while (_0x5f0163 > 0) {
      try {
...
smd({
  ...
  try {
    ...
    console.log(`API URL: ${`https://giftedapis.us.kg/api/download/ytdl?url=${videoUrl}&apikey=gifteddevskk`}`);
    const response = await axios.get(`https://giftedapis.us.kg/api/download/ytdl?url=${videoUrl}&apikey=gifteddevskk`);
...
`
    ...
    const apiUrl = `https://giftedapis.us.kg/api/download/ytmp4v2?url=${_0x4123ae.url}&apikey=gifteddevskk`;
    ...
    while (retries > 0) {
      try {
        ...
        // Check if the API call was successful and contains the video URL
        if (data.success && data.url) {
          const videoUrl = data.url; // The video download URL from the API

          // Fetch the video file from the video URL
          const videoResponse = await axios({
            url: videoUrl,
            method: 'GET',
            responseType: 'stream'
